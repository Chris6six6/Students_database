...
echo -e "$($PSQL "SELECT course FROM courses FULL JOIN majors_courses USING(course_id) RIGHT JOIN students USING(major_id) GROUP BY course HAVING COUNT(*) = 1 ORDER BY course")"